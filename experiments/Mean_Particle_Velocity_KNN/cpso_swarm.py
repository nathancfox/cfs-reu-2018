...
class COMB_Swarm:
    ...
    def __init__(self, npart, c1, c2, c3, ndim, alpha, test_size,
        ...
        self.var_by_time = {
                            ...
                            'velocities' : np.zeros((t_bounds[1], npart))
                           }
    ...
    def initialize_particles(self):
        ...
        for i in range(self.npart):
            ...
            self.var_by_time['velocities'][0, i] = (
                    self.v_extreme(self.swarm[i].v, self.v_bounds))
    ...
    def execute_search(self):
        ...
        for i in range(1, self.t_bounds[1]):
            ...
            for p in self.swarm:
                ...
                self.var_by_time['velocities'][i, counter] = (
                        self.v_extreme(self.swarm[counter].v, self.v_bounds))
    ...
    def v_extreme(self, v, bounds, lowerpercent=0.1, upperpercent=0.9):
        """Return percentage of v that is "extreme".
        ...
        None
        """
        upper = ((upperpercent * (self.v_bounds[1] - self.v_bounds[0]))
                   + self.v_bounds[0])
        lower = ((lowerpercent * (self.v_bounds[1] - self.v_bounds[0]))
                   + self.v_bounds[0]) 
        counter = 0
        for i in v:
