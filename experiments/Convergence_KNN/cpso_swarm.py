...
class COMB_Swarm:
    ...
    def __init__(self, npart, c1, c2, c3, ndim, terms, stagn_limit,
        ...
        self.var_by_time = {
                            ...
                            'spread_sum' : np.zeros(t_bounds[1]),
                            'spread_mean' : np.zeros(t_bounds[1])
                           }
    ...
    def initialize_particles(self):
        ...
        self.var_by_time['spread_sum'][0] = self.calc_spread(mean=False)
        self.var_by_time['spread_mean'][0] = self.calc_spread(mean=True)
        self.init_flag = True
    ...
    def execute_search(self):
        ...
        for i in range(1, self.t_bounds[1]):
            ...
            self.var_by_time['spread_sum'][i] = self.calc_spread(mean=False)
            self.var_by_time['spread_mean'][i] = self.calc_spread(mean=True)
