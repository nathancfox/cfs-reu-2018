...
def single_test(clf, b, data, target):
    cm = np.zeros((2, 2))
...
def main():
    ...
    scores['kfold'].append((0, kfold_test(clf, 10,
                                          np.isin(labels, list(cohorts[0]())),
                                          data, target)))
    scores['kfold'].append((1, kfold_test(clf, 10,
                                          np.isin(labels, list(cohorts[1]())),
                                          data, target)))
    scores['kfold'].append((2, kfold_test(clf, 10,
    ...
    scores['single'].append((0, single_test(clf,
                                            np.isin(labels, list(cohorts[0]())),
                                            data, target)))
    scores['single'].append((1, single_test(clf,
                                            np.isin(labels, list(cohorts[1]())),
                                            data, target)))
    scores['single'].append((2, single_test(clf,
                                            np.isin(labels, list(cohorts[2](linked_groups))),
                                            data, target)))
    scores['single'].append((2, single_test(clf,
                                            np.isin(labels, list(cohorts[2](linked_groups))),
    ...
    with open('summary_report.txt', 'w') as f:
        ...
        f.write('#'*100+'\n')
        f.write('#\n')
        f.write('# {}\n'.format('Verifying Rational Polypharmacology - KNN'))
        f.write('# {}\n'.format('Nathan Fox'))
